...
const docker = {
    async run(image, commandArgs, options, execOptions, debug) {
    ...
    async build(url, options, debug) {
        if (debug === true && options.progress == null)
    ...
    async images(tag, options) {
        options = Object.assign({format: 'json'}, options);
        const child = await docker.command('images', tag, options);
        return JSON.parse(child.stdout || null);
    },

    async tag(source, target) {
        await docker.command('tag', [source, target]);
    },

    async command(command, args, options, execMode, execOptions, debug) {
        ...
        if (execMode == 'spawn') {
            ...
            const child = spawn(dockerBin, execArgs, execOptions);
...
class Container {
    ...
    async exec(options, command, commandArgs, execMode, execOptions, debug) {
        let args = [this.id, command];
        if (commandArgs) args = args.concat(commandArgs);
        await docker.command('exec', args, options, execMode, execOptions, debug);
    }
}
