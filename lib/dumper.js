...
module.exports = class Dumper {
    ...
    async init(dumpDir, dumpFile) {
        ...
        this.dumpPath = path.join(dumpDir, `${dumpFile}.sql`);

        // FIXME: If it's called after docker.build() statement it creates an 
        // "invalid" WriteStream
        const dumpStream = await fs.createWriteStream(this.dumpPath);
    ...
    async dumpFixtures(tables, replace, args) {
        ...
        await this.dumpStream.write('SET FOREIGN_KEY_CHECKS=0;\n', 'utf8');
        ...
        await this.dumpStream.write('SET FOREIGN_KEY_CHECKS=1;\n', 'utf8');
    }
