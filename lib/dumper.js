...
module.exports = class Dumper {
    ...
    async init(dumpDir, dumpFile) {
        ...
        const buidDir = path.join(__dirname, '../docker/client');
    ...
    async runDump(command, args) {
        const iniPath = path.join('remotes', this.ctx.iniFile);
        const myArgs = [
        ...
        await docker.run('myt/client', commandArgs, {
            ...
            volume: `${this.ctx.mytDir}:/workspace`,
            rm: true
        }, execOptions, this.cfg.debug);
    }
}
